...
void loop()
{
  ...
  mcu_ain_voltage2 = mcu_ain_voltage * 65535.0; //solution to convert to int
  mph_float = average_raw * .0594 - 6.67 - 1.104;
//  if(mph_float <= 2.512400)
//  {
//    mph_float = 0.0;
//  }
  mph = (int)mph_float; //solution to convert to int
//
//  Serial.printf("mph float = %f \n", mph_float);
  Serial.printf("mph = %i \n", mph);

  //voltage_sensor = mcu_ain_voltage / 0.6;     //WisBlock RAK5811 (0 ~ 5V).   Input signal reduced to 6/10 and output
//  Serial.printf("math op: %f\n", mcu_ain_voltage * 65535.0);
...
//  Serial.printf("-------voltage_sensor Int------ = %i\n", mcu_ain_voltage2); //int conversion
...
void bme680_get()
{
  ...
  m_lora_app_data.buffer[i++] = (mph >> 8) & 0xFF;
  m_lora_app_data.buffer[i++] = mph & 0x00FF;
